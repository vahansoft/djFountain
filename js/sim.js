...
Simulator.prototype = {
    ...
    light: function (id) {
        if(!this.lightList[id]){alert("No such light: "+id);}
    ...
    display: function (time) {
        ...
        if(!CONFIG.ShowOn)
        {
        	 if (time) 
             {
        		// self.Register.registerSleep(time);
        		// self.Register.registerDisplay();
        ...
        else
        {
	        ...
	        if (time) 
	        {
	            ...
	            setTimeout(function () 
	            {
	              //  self.Register.registerSleep(time / 1000);
	                self.renderAll();
	               // self.Register.registerDisplay();
	            }, dTime);
    ...
    sendData: function () {
        ...
        function saveFilePrompt()
        {
        	...
        	if(name)
            {
	        	$.post("handle.php",{save:1,name:name,music:musicFile},function(r){
	        		 alert(r);
	        	});
        	}
        }
                    ...
                    var proc =  pInterator/fullLen*100;
             	 	...
             	 	$('.bar').css('width',proc+'%').text(Math.round(proc)+'%');
             	 	 ...
             	 	 if(chunk[SendIterator])
             	 	 {
		             	    ...
		             	    if(SendIterator == fullLen-1){_last=true;}
    ...
    setColor: function (r, g, b) {
        ...
        if (g == null) {
            ...
            this.sim.addFunction(function () {
                ...
                //self.content.set('fill', r.toRgba());
                self.color = r.toRgba();
              //  self.sim.Register.registerColor(self.id, r.r, r.g, r.b);

            });
        } else {
            self.realColor = makeColor(r, g, b).realValue;
            this.sim.addFunction(function () {
                //self.content.set('fill', c(r, g, b).toRgba());
                self.color = makeColor(r, g, b).toRgba();
